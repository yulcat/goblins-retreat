...
report.revision=process.env.TEST_LABEL||'fullscreen-painted-rush';report.url=process.env.TEST_URL||'http://127.0.0.1:4173';await writeFile(process.env.REPORT_PATH||(process.env.TEST_URL?'reports/revision-hosted-smoke.json':'reports/revision-smoke.json'),JSON.stringify(report,null,2));console.log(report);await browser.close();assert.equal(errors.length,0);
