...
for(const mobile of [false,true]){
 ...
 const fixture=createGame(813);fixture.towers=[makeTower(fixture,'harpoon')];fixture.seen=Object.keys(SCENES);fixture.rewards=18;
 // A single unplaced weapon isolates firing on the real route; this is not a completion test.
 ...
 await click('#continue-game');await page.clock.runFor(80);await click(`[data-item="${fixture.towers[0].id}"]`);await cell(2,2);await click('#confirm-place');await page.clock.runFor(80);
 ...
 await click('#rotate');await click('#rotate');await cell(2,2);await click('#confirm-place');await page.clock.runFor(80);
 ...
 await click('#rotate');await click('#rotate');await cell(2,2);await click('#confirm-place');
 ...
 await click('#resume');await page.clock.runFor(80);await cell(2,2);
...
const report={created:new Date().toISOString(),url,scope:'Single-weapon regression fixture. Placement, rotation and combat selection use real pointer input; enemies and hits run through the normal simulation.',results,errors};await writeFile(process.env.TEST_URL?'reports/harpoon-hosted.json':'reports/harpoon-browser.json',JSON.stringify(report,null,2));console.log(report);await browser.close();assert.deepEqual(errors,[]);
