...
export function drawWeapon(c,type,x,y,angle=0,time=0){
 ...
 if(type==='nail'){
  ...
  // Stock and drawn string sit behind the forward bow; the loaded tip points out.
  path(c,'M-.67 -.1 L-.42 -.18 L.54 -.13 L.67 0 L.54 .13 L-.42 .18 L-.67 .1Z',metal(c,-.6,-.18,1.3,.36,'#a7b2a1','#3d5045'));
  plate(c,-.64,-.105,.35,.21,'#947548');plate(c,-.3,-.065,.9,.13,'#1b2d28');
  path(c,'M.14 -.53 Q.59 -.4 .5 0 Q.59 .4 .14 .53',null,'#263c31',.10);
  path(c,'M.14 -.53 Q.59 -.4 .5 0 Q.59 .4 .14 .53',null,'#afbd9f',.055);
  line(c,.14,-.53,-.38,0,'#ead5a1',.023);line(c,-.38,0,.14,.53,'#ead5a1',.023);
  plate(c,.43,-.12,.14,.24,'#718878');bolt(c,.5,0,.028);
  line(c,-.4,0,.9,0,'#e4e8cb',.052);
  path(c,'M-.4 0 L-.53 -.13 L-.28 -.07 L-.2 0 L-.28 .07 L-.53 .13Z','#b37b51','#443d2b',.012);
  path(c,'M.76 -.035 L.72 -.15 L1.04 0 L.72 .15 L.76 .035Z','#e5ecd7','#263b32',.025);
  line(c,.78,0,1.01,0,'#fff4cc',.015);bolt(c,-.52,.075,.025);
 }else if(type==='saw'){
