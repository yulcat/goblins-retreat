...
export class BoardRenderer{
 ...
 const editing=['garage','reward'].includes(s.phase);
 ...
 if(selection===t.id){ellipse(c,o.x,o.y,(d.range||.5)+modifiers(s,t).range,(d.range||.5)+modifiers(s,t).range,'#eed29e13','#f5d08eaa',.025);for(const [x,y] of cells(t))plate(c,x+.04,y+.04,.92,.92,'#c3b17433','#f1d597');}
 drawWeapon(c,t.type,o.x,o.y,d.directional?t.r*Math.PI/2:t.aim,s.phase==='combat'?s.time:0);
 if(d.directional&&(selection===t.id||editing)){const dir=[[1,0],[0,1],[-1,0],[0,-1]][t.r];line(c,o.x+dir[0]*.8,o.y+dir[1]*.8,o.x+dir[0]*1.25,o.y+dir[1]*1.25,'#efcd83',.034);}
 ...
 else if(f.type==='beam'){line(c,f.x,f.y,f.tx,f.ty,'#e5ddb833',.08);const x=f.x+(f.tx-f.x)*Math.min(1,age*3),y=f.y+(f.ty-f.y)*Math.min(1,age*3);c.translate(x,y);c.rotate(Math.atan2(f.ty-f.y,f.tx-f.x));line(c,-.6,0,.1,0,'#cfd9c4',.055);path(c,'M.03 0 L.14 -.09 L.29 0 L.14 .09Z','#e1e6d0',null);}
